...
with open(r'c:\Parisu Ulagam\frontend\src\components\CustomerDashboard.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Add states and functions
states_str = '''  const [profile, setProfile] = useState(null);
...
new_states_str = '''  const [profile, setProfile] = useState(null);
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [activeTab, setActiveTab] = useState('profile');
...
# Sidebar and Overlay
sidebar_str = '''    <div className="admin-layout" style={{ zIndex: 9999, position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'var(--bg-primary)' }}>
      <aside className="admin-sidebar">'''

new_sidebar_str = '''    <div className="admin-layout" style={{ zIndex: 9999, position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'var(--bg-primary)' }}>
...
header_title_str = '''<h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '300' }}>'''
new_header_title_str = '''<div style={{ display: 'flex', alignItems: 'center' }}>
...
header_title_close_str = '''</h1>
          </div>'''
new_header_title_close_str = '''</h1>
            </div>
          </div>'''
content = content.replace(header_title_close_str, new_header_title_close_str)

# Update onClick handlers in nav
content = content.replace("setActiveTab('profile')", "setActiveTab('profile'); setIsSidebarOpen(false);")
content = content.replace("setActiveTab('wishlist')", "setActiveTab('wishlist'); setIsSidebarOpen(false);")
content = content.replace("setActiveTab('cart')", "setActiveTab('cart'); setIsSidebarOpen(false);")
content = content.replace("setActiveTab('orders')", "setActiveTab('orders'); setIsSidebarOpen(false);")
content = content.replace("setActiveTab('queries')", "setActiveTab('queries'); setIsSidebarOpen(false);")

# Profile UI update
profile_ui_str = '''{/* PROFILE */}
          {activeTab === 'profile' && (
...
with open(r'c:\Parisu Ulagam\frontend\src\components\CustomerDashboard.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated CustomerDashboard.jsx successfully.")
